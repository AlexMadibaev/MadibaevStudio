...
type LocalizedList = Record<MgsLocale, string[]>;
...
const serviceDefinitions: readonly ServiceDefinition[] = [
  {
    slug: "branding",
...
export function MgsWorkPage({ locale }: { locale: MgsLocale }) {
  ...
  return (
    <>
      <MgsDocumentLocale locale={locale} title={`MGS — ${copy.title}`} description={copy.description} />
...
export function MgsServiceDetailPage({ locale, slug }: { locale: MgsLocale; slug: string }) {
  ...
  return (
    <>
      <MgsDocumentLocale locale={locale} title={`MGS — ${service.title}`} description={service.summaryText} />
      <main className="mgs-route-page mgs-shell">
        <section className="mgs-route-hero">
          <p className="mgs-eyebrow">
            <span />
