...
export class MgsContentStoreUnavailableError extends Error {
  constructor(message = "Persistent content storage is not configured.") {
    super(message);
...
function hasPersistentStorage() {
  return Boolean(getLocalDataRoot() || process.env.BLOB_READ_WRITE_TOKEN?.trim());
}

function getLocalStoragePath(pathname: string) {
...
async function readStorageText(pathname: string) {
  const local = getLocalDataRoot();

  if (local) {
    const { target } = getLocalStoragePath(pathname);

    try {
      const text = await readFile(target, "utf8");
      return { text, etag: hashText(text) };
    } catch (error) {
...
async function readPrivateJson<T>(
  ...
  try {
    const raw = await readStorageText(pathname);
...
async function writePrivateJson(pathname: string, value: unknown, etag?: string | null) {
  ...
  if (local) {
    if (etag) {
      const current = await readStorageText(pathname);
      if (!current || current.etag !== etag) {
        throw new Error("Stored data changed before this save. Reload and try again.");
      }
    }
...
async function listLocalStorageObjects(prefix: string): Promise<StorageObject[]> {
  const { root, target } = getLocalStoragePath(prefix);
  ...
  async function walk(directory: string) {
    ...
    await Promise.all(
      entries.map(async (entry) => {
        ...
        objects.push({
          pathname: relative(root, absolute).split(sep).join("/"),
          etag: hashText(text),
        });
      }),
    );
...
async function listStorageObjects(prefix: string): Promise<StorageObject[]> {
  ensureStorageConnected();
  ...
  return blobs.map((blob) => ({ pathname: blob.pathname, etag: blob.etag }));
}

async function deleteStorageObject(pathname: string, etag?: string | null) {
  ensureStorageConnected();
  ...
  if (getLocalDataRoot()) {
    if (etag) {
      const current = await readStorageText(pathname);
      if (!current || current.etag !== etag) {
        throw new Error("Stored data changed before this delete. Reload and try again.");
      }
    }
...
async function readStoredAdminProjects() {
  if (!hasPersistentStorage()) {
    return {
      ...
      status: {
        connected: false,
        issue: "Persistent storage is not configured. Set MGS_DATA_DIR for self-hosting or BLOB_READ_WRITE_TOKEN on Vercel.",
...
export async function listMgsEnquiries() {
  if (!hasPersistentStorage()) {
    return {
      ...
      status: {
        connected: false,
        issue: "Persistent storage is not configured. New enquiries cannot be saved yet.",
      } satisfies MgsContentStoreStatus,
    };
  }

  const objects = await listStorageObjects(ENQUIRIES_BLOB_PREFIX);
  const pendingRecords = await Promise.all(
    objects.map(async (object): Promise<MgsEnquiry | null> => {
      const result = await readPrivateJson(object.pathname, normalizeEnquiry);
...
export async function removeMgsEnquiry(id: string) {
  ...
  await deleteStorageObject(enquiry.blobPathname, enquiry.etag);
}
